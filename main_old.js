...
class IpAddress {
  ...
  let ipv4MappedIpv6 = null;
    if (!cidr.isValid()) {
      ipv4MappedIpv6 = res.json (`"IPv4":"${firstIpAddress}" , "IPv6":"${ipv4MappedIpv6}"`)
    } else {
      ipv4MappedIpv6 = res.json (`"IPv4":"${firstIpAddress}" , "IPv6":"` + getIpv4MappedIpv6Address(firstIpAddress) +'"');
    }
